...
### 2. 效果评测：66 条标注用例 + 三方案消融

回归测试过了只能说明「没坏」，说明不了「答得准」。`backend/eval/` 回答后者。

同一套 66 条用例跑三种配置，**A→B 只加工具，B→C 只加约束**，差值即各自贡献：
...
实测（Sep-16 baseline，temperature=0.3）：
...
| **C 工具 + 边界约束** | **98.3%** | **0.0%** | **0.0%** | **0.0%** | **0.0%** | **100%**¹ | 2,367 ms | 2,054 / 233 |
...
> ¹ 工具命中率 98.2% → **100%** 是 P1-2 的结果：聚合题 G47（「饮料里最便宜的是哪个？」）从误走
> `search_product` 改为正确走 `query_product_database` 做 MIN 聚合，详见本节第 4 点。注意工具命中率与
> 事实正确率是两个指标——P1-2 修的是工具选择，事实正确率因 DeepSeek temp 0.3 非确定性在 93–98% 间波动，
> 小样本下不把单点差异当结论（见 `backend/eval/README.md` 第七节）。聚合类评测 P0 已从 1 条扩到 10 条
> （MIN/MAX/COUNT/排名第/排序 Top-3/跨商品比价），66 题重跑下 guard 组 10/10 事实正确 + 10/10 工具命中，
> 证明修复泛化（见 `backend/eval/README.md` 第八节）。
...
**人工试用完全看不出来**；只有拿 66 条题一条条过，5 道库存题全挂才暴露。
...
| **现线上**（两路加权求和） | 92.3% | 94.2% | 100% |
| 两路 RRF（字面 + 属性，无向量） | 82.7% | 92.3% | 100% |
| 三路加权求和（+ 向量） | 92.3% | 96.2% | 100% |
| 纯向量（text-embedding-v3） | 94.2% | 98.1% | **0%** |
| 三路 RRF（向量也参与放行） | 96.2% | 96.2% | **0%** |
| **三路 RRF + 结构化闸门** | **96.2%** | **96.2%** | **100%** |

三个发现：

1. **口径选错会得出相反的建议。** 如果把顾客原话直接喂检索层，纯向量 92.3% 对现线上 84.6%，
   看起来「该换成向量」；但换成线上真实的短关键词后，差距只有 **+1.9pp**——
   向量的强项是扛住长口语问句（「抽纸在哪个区？」），而线上模型传进来的本来就是「抽纸」，
   这项优势被上游的查询改写提前吃掉了。
2. **「向量路」与「RRF 融合」必须配套**（这条结论被自己的下一组数据推翻后修正过）：
   ...
   加权求和下加向量路**毫无变化**（等权归一化把余弦的区分度稀释了）；
   RRF 下加向量路 **+13.5pp**（向量单路 R@1 94.2% 是全场最高的排序质量，
   而 RRF 靠排名投票、不靠分数求和，正好用得上它）；RRF 在两路时反而比加权求和差 9.6pp。
   → 单加向量无效、单换融合有害，**两者配套**才拿到 96.2%。
3. **向量不能兼任「库里有没有这件东西」的闸门。** 纯向量召回最高，却一条库外题都挡不住
   （「iPhone」→ 卫龙亲嘴烧、奥利奥、蒙牛雪糕）。扫阈值可见：库外题最高余弦 0.530、
   库内题最低 0.475，两条分布重叠；要把库外题全挡住得把阈值提到 0.60，那时库内题只剩 40/52 能过。
   → 召回交给多路融合，**闸门只能由结构化信号把关**。
4. **一条被数据否掉的方案**：「结构化闸门 + 强向量补召回」与不带强向量版全部指标完全相同，
   0.60 那条路径没有任何增量，是应该删掉的复杂度。负结果留在报告里，免得下次再想一遍。

> ⚠️ 上面的实验结论**没有落到线上代码**——这是算过账后的决定，不是遗留项：
> 端到端评测里与检索召回相关的失分是 **0 条**（C 组 12 个类别中 11 个满分，唯一失分是
> 「模型没走 SQL 兜底做聚合」——这是**工具选择**问题、**不是**检索问题，已在 P1-2 通过改提示词修复，
> 见本节第 4 点），检索层 +3.9pp 换不来端到端收益，而代价是每次检索多一次
> embedding 调用（+100~300ms）与一个外部故障点。**组件指标 ≠ 产品指标**，
> 这条判断本身比那 3.9pp 更值得讲。等 SKU 规模上去（几百个）或检索质量本身成为交付指标时再落地。
...
### 4. P1-2：工具选择修复（聚合题误走 search_product）

第六节结论四挖出的真短板 **G47「饮料里最便宜的是哪个？」** 线上组用 `search_product` 答错
（把可口可乐当成最便宜），根因是**工具选择**而非检索召回——`search_product` 一次只返回按相关度
排序的前几件，根本做不了 MIN / 排序 / 聚合。本节点清理并修复它。

**量化**：66 条里需「算整张表」的聚合 / 比较 / 排序类，原 baseline 仅 2 条（G47 MIN、G48 COUNT），**P0 已扩到 10 条**（G47/G48 + G59–G66，见 `backend/eval/README.md` 第八节）；
原 baseline 真正因工具选择答错的仅 G47 这 1 条（G48 本就走 SQL）。工具命中率 0.9815 → **1.0**。

**修复**（`backend/app/prompts/guide.yaml` 两处）：

1. rule #6 从「SQL 是兜底、预设能答就别写」改为「工具选型原则」——单商品属性走预设工具，
   **算整张表（最便宜 / 最贵 / 价格排序 / 共多少款 / 均价 / 排名第 / 跨商品比价）必须直接写 SQL**；
2. `query_product_database` 工具描述从「仅当预设都答不了才用」改为显式列举计算型场景。

**验证（诚实标注方差）**：修复后 G47 稳定走 `query_product_database` 并答对（农夫山泉 ¥2.0）。
但 DeepSeek temp 0.3 下非确定性，单条重跑前后对比会被方差淹没；同日旧/新双版本对照显示单题
**随机翻面**，端到端在 93–98% 波动（Sep-16 baseline 98.3%）。**G47 在旧版同日重跑里也碰巧走 SQL 答对**，
进一步说明它受方差影响。结论：修复方向正确、无可追溯回归，但原 baseline 仅 58 条、聚合类只有 1 条（G47），
小样本无法统计证净增益；**P0 已将聚合类扩到 10 条，guard 组 10/10 事实正确 + 10/10 工具命中，
泛化到全部聚合类型得到验证**（见 `backend/eval/README.md` 第八节）——
与「小规模只报趋势不报结论」口径一致，修复仍保留。对照原始记录归档在 `eval/results/baseline_pre_p1_2/`。

> 方法论：**验证 LLM 改动不要靠「一次重跑的前后对比」**（temp>0 单题方差淹没真实效应），
> 要做同日、同输入、旧/新双版本对照，且样本量足够。这条呼应 P1-1——看产品指标分类别拆解、承认小样本局限。
