...
改为**闭卷选择**后，28 个 SKU 的清单（ID + 名称 + 规格 + 包装形态 + 外观特征）直接注入提示词，模型只输出 `sku_id`，关键词→检索这个错配环节被彻底移除。清单外的 `sku_id` 一律丢弃。
...
- **从全部商品指定** —— 模型连候选都给不准时，在 28 个 SKU 中直接指定。
